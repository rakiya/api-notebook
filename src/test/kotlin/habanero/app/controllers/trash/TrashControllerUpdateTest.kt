...
class TrashControllerUpdateTest : ControllerTest() {
    ...
    @BeforeEach
    fun beforeEach() {
        listOf(NOTEBOOK, PAGE, SECTION).forEach {
            db.deleteFrom(it).execute()

        }

        listOf(notebooks, pages, sections).forEach { entities ->
            db.batchInsert(entities.map { it.toRecord() }).execute()
        }
    }

    @Test
    @DisplayName("セクションをゴミ箱に移動できること")
    fun `You can throw away your section`() {
        val target = getSectionOwnedBy(myAccount)[0]

        post<NoContentResponse, Unit>("/trash/${target.sectionId}", null, myAccount) {
            validateWhenSuccess(target.toRecord(), it.statusCode)
        }
    }

    @Test
    @DisplayName("既にゴミ箱にあるセクションに対して操作できること")
    fun `You can throw away your section in the trash`() {
        val target = getSectionOwnedBy(myAccount)[0]
    ...
    fun `You should have logged in`() {
        ...
        post<NoContentResponse, Unit>("/trash/${target.sectionId}", null) {
    ...
    fun `You cannot throw away others' section`() {
    ...
    fun `You cannot throw away a section not existed`() {
        post<ErrorResponse, Unit>("/trash/9999", null, myAccount) {
    ...
    private fun validateWhenSuccess(expected: SectionRecord, actualStatusCode: Int) {
        Assert.assertEquals(StatusCode.ACCEPTED_CODE, actualStatusCode)
        ...
        Assert.assertEquals(1, actual.isTrashed.toInt())
